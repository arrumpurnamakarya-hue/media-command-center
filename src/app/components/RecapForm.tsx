...
interface PreviewItem {
  ...
  views: number;
  engagement: number;
  publish_date: string; // TAMBAHAN KUNCI: Tanggal
}
...
export default function RecapForm({ isDarkMode = true, onRecapSuccess }: RecapFormProps) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, platformKey: string) => {
    ...
    reader.onload = (event) => {
      ...
      for(let i = 1; i < parsedRows.length; i++) {
        ...
        let views = 0;
        let engagement = 0;
        let pDate = new Date().toISOString().split('T')[0]; // Default hari ini
        ...
        if (platformKey === 'web') {
          ...
          // PEMBACA TANGGAL META (FB/IG) format: MM/DD/YYYY
        ...
        if(caption && caption !== "0" && (views > 0 || engagement > 0)) {
          parsedData.push({
            ...
            publish_date: pDate // Masukkan ke array
          });
        }
      }
    ...
    try {
      for (const item of previewData) {
        ...
        const payload: Record<string, any> = {
          ...
          publish_date: item.publish_date, // SUDAH MEMAKAI TANGGAL DARI CSV
          publish_time: '12:00',
          views: item.views,
  ...
  const CSVUploadZone = ({ title, icon, dataKey }: { title: string, icon: React.ReactNode, dataKey: keyof typeof platformStatus }) => {
    ...
    return (
      <div className={`p-5 rounded-2xl border transition-all duration-300 relative overflow-hidden ${platData.status === 'success' ? 'bg-emerald-500/10 border-emerald-500/30' : isDarkMode ? 'bg-[#0b0d10] border-gray-800' : 'bg-gray-50 border-gray-200'}`}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">{icon}<h4 className={`text-xs font-black uppercase tracking-wider ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{title}</h4></div>
          {platData.status === 'success' && <button onClick={() => handleRemoveFile(dataKey)} className="text-gray-500 hover:text-red-400 p-1 rounded transition-colors"><XIcon size={14}/></button>}
        </div>
        {platData.status === 'idle' ? (
          ...
          <div className="p-6 flex flex-col items-center justify-center"><RefreshCw className="w-6 h-6 text-emerald-500 animate-spin mb-3" /><span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest animate-pulse">Membaca File CSV...</span></div>
        ) : (
          <div className="p-4 bg-black/20 rounded-xl border border-emerald-500/20 flex flex-col items-center text-center"><CheckCircle2 className="w-8 h-8 text-emerald-500 mb-2" /><span className="text-[10px] text-emerald-400 font-bold truncate max-w-full px-2">{platData.file}</span><span className="text-[8px] text-gray-500 uppercase mt-1">Data Siap Disinkronkan</span></div>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className={`${isDarkMode ? 'bg-[#12151a] border-gray-800' : 'bg-white border-gray-200'} border p-8 md:p-10 rounded-[35px] shadow-2xl`}>
        <div className="text-center mb-10"><h2 className={`text-2xl font-black uppercase tracking-tight ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Smart Bulk Sync Engine</h2><p className="text-xs text-gray-400 mt-1 uppercase tracking-widest font-bold">Unggah CSV, Sistem Akan Mengimpor Data Historis Secara Otomatis</p></div>
        ...
        {previewData.length > 0 && (
          <div className={`p-6 rounded-3xl border animate-fadeIn mb-8 ${isDarkMode ? 'bg-[#0b0d10] border-gray-800' : 'bg-gray-50 border-gray-200'}`}>
            <div className="flex justify-between items-center mb-6"><div><h4 className={`text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Data Asli dari CSV</h4><p className="text-[10px] text-gray-500 mt-1 font-bold">Memuat {previewData.length} baris data historis yang siap diimpor ke Reports</p></div></div>
              ...
              <table className="w-full text-left">
                <thead className={`sticky top-0 z-10 ${isDarkMode ? 'bg-[#0b0d10]' : 'bg-gray-50'}`}>
                  <tr className="text-[10px] text-gray-400 uppercase border-b border-gray-500/20 pb-2">
                    ...
                    <th className="pb-3 px-4 font-black text-right">Reach</th>
                    <th className="pb-3 px-4 font-black text-right">Total Eng.</th>
                  </tr>
                </thead>
                <tbody className="text-[11px] font-bold divide-y divide-gray-500/10">
                  {previewData.map((d, i) => (
                    <tr key={i} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className={`py-4 px-4 max-w-[250px] leading-relaxed ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>{d.title}</td>
        ...
        {errorMessage && (
          <div className="mb-6 p-4 rounded-2xl bg-red-500/10 border border-red-500/30 flex items-start gap-3 animate-fadeIn"><AlertTriangle className="text-red-500 flex-shrink-0" size={18} /><div><h4 className="text-xs font-black text-red-500 uppercase tracking-widest">Koneksi Database Ditolak</h4><p className="text-[10px] text-red-400/80 mt-1 font-mono">{errorMessage}</p></div></div>
        )}

        <button onClick={handleBulkSave} disabled={previewData.length === 0 || isSubmitting} className={`w-full flex items-center justify-center gap-2 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${previewData.length === 0 || isSubmitting ? 'bg-[#12151a] border border-gray-800 text-gray-600 cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-900/50 cursor-pointer active:scale-[0.98]'}`}>
          {isSubmitting ? <><RefreshCw className="w-4 h-4 animate-spin" /> MENYIMPAN DATA...</> : previewData.length === 0 ? <><AlertTriangle size={14} /> TOMBOL TERKUNCI (UNGGAH CSV DULU)</> : <><Save size={16} /> IMPOR MASSAL KE REPORTS</>}
        </button>
      </div>
    </div>
  );
}
function XIcon(props: any) { return <svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg>; }